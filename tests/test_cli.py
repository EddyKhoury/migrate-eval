...
def test_cli_workers_run_problems_concurrently(
    tmp_path,
    monkeypatch,
):
    problems = [
    ...
    def fake_migrate(
        ...
        *,
        ...
        # This succeeds only if both problems are actually
...
def test_parallel_cost_limit_stops_new_problem_submission(
    tmp_path,
    monkeypatch,
):
    problems = [
        MigrationProblem(
            task_id=f"Go/{index}",
            java_code=f"JAVA {index}",
            go_signature="func Example() int {",
            go_test="package main",
        )
    ...
    # Two tasks may already be in flight when the first
    # completed task pushes the run over the soft limit.
    # No third or fourth task should be submitted.
    assert len(executed) == 2
    assert set(executed).issubset(
        {
            "Go/0",
            "Go/1",
        }
